...
class Field:
    def __init__(self, grid_resolution=32, initial_density=1.0, rng=None):
        ...
        self.nodes = []
        self.rng = rng
        # create simple nodes with phases
        N = grid_resolution * grid_resolution
        for i in range(N):
            # initialize small random phase offsets for determinism if rng provided
            phase = float(rng.normal(0,0.001)) if rng is not None else 0.0
            self.nodes.append({'phase': phase, 'density': initial_density})
        # create few anchor clusters for demo (split nodes roughly)
        split = max(1, N//10)
    ...
    def add_phase_noise(self, delta):
        import numpy as np
        for n in self.nodes:
            noise = float(self.rng.normal(0, delta*0.1)) if self.rng is not None else (np.random.randn()*delta*0.1)
            n['phase'] += delta + noise
